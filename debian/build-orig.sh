...
version=$( dpkg-parsechangelog -l`dirname $0`/changelog | sed -n 's/^Version: \(.*:\|\)//p' | sed 's/-[0-9.]\+//' | sed 's/+zentyal[0-9]\+//' )
...
if echo "$version" | egrep "(bzr|git)" >/dev/null; then
	...
	upstream_version=`echo $version | sed -e 's/~rc/rc/;s/~alpha1~tp/tp/;s/~beta/beta/;s/~alpha/alpha/;s/.dfsg[0-9]*$//;'`
	wget ftp://ftp.samba.org/pub/samba/rc/samba-$upstream_version.tar.gz
	wget ftp://ftp.samba.org/pub/samba/rc/samba-$upstream_version.tar.asc
